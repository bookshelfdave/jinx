...
func TestSimpleChar(t *testing.T) {
...

	{
		...
		p0 := Char('1').PipeGen(func(s interface{}) interface{} {
			str := s.(string)
...
func TestSimpleSeq(t *testing.T) {
	{
		...
		p1 := Char('2')
		p2 := Char('3')
		s := Seq(p0, p1, p2).PipeGen(GenString)
		result := s.Parse(ps)
		if result.Success == false {
			t.Error("Expected parse success")
		}
...
func TestStrSeq(t *testing.T) {
	...
	alt := Seq(Str("foo"), Str("bar"), Alt(Str("BAZ"), Str("baz"))).PipeGen(GenString)
	result := alt.Parse(ps)
	if result.Success == false {
		t.Error("Expected parse success")
	}
...
func TestAttempt(t *testing.T) {
	{
		...
		m := Alt(Attempt(fc), ab).PipeGen(GenString)
		result := m.Parse(ps)
...
func TestSepBy(t *testing.T) {
...

	{
		...
		ps.ParserFromString("[1,2,3,4]")
		digits := Between(Char('['),
			SepBy(Digit(), Char(',')).PipeGen(GenString, GenStringToInt),
			Char(']'))
		result := digits.Parse(ps)
		if result.Success == false {
			t.Error("Expected parse success")
...
/*func TestParseCSV(t *testing.T) {
	{
		...
		// Length *includes* the commas + whitespace
		if result.Length != 6 {
			t.Error("Expected Length == 5")
		}
	}

	{
		...
		header := Seq(SepBy(Word(), Char(',')).PipeGen(GenListOfStrings), IgnoreWS()).PipeGen(GenSelect1(0))
		line := Seq(SepBy(Number(), Char(',')).PipeGen(GenString), IgnoreWS())
		csv := Seq(header, Many(line).PipeGen(GenIdentity))
		result := csv.Parse(ps)
		if result.Success == false {
			t.Error("Expected parse success")
		}
